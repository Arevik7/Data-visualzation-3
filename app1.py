...
@app.callback(
	[Output(component_id='checklist-weights-three-categories-2', component_property='options'),
	Output(component_id='checklist-weights-three-categories-2', component_property='value')],
	[Input(component_id='checklist-weights-three-categories-1', component_property='value')])
...
@app.callback(
	 Output(component_id='survival-function', component_property='figure'),
	 [Input(component_id='checklist-three-categories-survival-function', component_property='value')])
def survive(drug_list):
	categories = {'lightweight': ['Capomulin','Ramicane'],
					'placebo': ['Placebo'],
					'heavyweight': ['Ceftamin', 'Infubinol', 'Ketapril',
								'Naftisol', 'Propriva', 'Stelasyn','Zoniferol']}
	output_options = []
	for category in drug_list:
		output_options.extend(categories[category])
	traces = []
	for drug_name in output_options:
		traces.append(go.Scatter(x=merged_df[merged_df['Drug Regimen']==drug_name].groupby('Timepoint').count().index.values,
								  y = merged_df[merged_df['Drug Regimen']==drug_name].groupby('Timepoint').count()['Mouse ID'].values,
									name = drug_name,
...
def survive(selected):
	drug_list = ['Placebo']
	if selected:
		drug_list = [selection['customdata'][0] for selection in selected['points']]
	traces = []
	for drug_name in drug_list:
...
def survive(selected):
	...
	for drug_name in drug_list:
		traces.append(go.Scatter(x=merged_df[merged_df['Drug Regimen']==drug_name].groupby('Timepoint').count().index.values,
								  y =  merged_df[merged_df['Drug Regimen']==drug_name].groupby('Timepoint').count()['Metastatic Sites'].values,
									name = drug_name,
									mode = 'lines + markers',
									marker = dict(color=drug_colors[drug_name])
						)
